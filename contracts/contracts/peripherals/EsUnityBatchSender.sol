...
contract EsUnityBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esUnity;

    constructor(address _esUnity) public {
        admin = msg.sender;
        esUnity = _esUnity;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsUnityBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esUnity).transferFrom(msg.sender, _accounts[i], _amounts[i]);
