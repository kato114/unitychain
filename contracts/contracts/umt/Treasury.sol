...
contract Treasury is ReentrancyGuard, ITimelockTarget {
    ...
    uint256 public umtPresalePrice;
    uint256 public umtListingPrice;
    ...
    function initialize(
        ...
        umtPresalePrice = _values[0];
        umtListingPrice = _values[1];
    ...
    function swap(uint256 _busdAmount) external nonReentrant {
        ...
        // send UMT
        uint256 umtAmount = _busdAmount.mul(PRECISION).div(umtPresalePrice);
        IERC20(umt).transfer(account, umtAmount);
    }
    ...
    function addLiquidity() external onlyGov nonReentrant {
        ...
        uint256 umtAmount = busdAmount.mul(PRECISION).div(umtListingPrice);

        IERC20(busd).approve(router, busdAmount);
        IERC20(umt).approve(router, umtAmount);

        IUMT(umt).endMigration();

        IPancakeRouter(router).addLiquidity(
            busd, // tokenA
            umt, // tokenB
            busdAmount, // amountADesired
            umtAmount, // amountBDesired
            0, // amountAMin
            0, // amountBMin
            address(this), // to
