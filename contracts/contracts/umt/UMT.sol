...
import "./interfaces/IUMT.sol";
import "../peripherals/interfaces/ITimelockTarget.sol";

contract UMT is IERC20, IUMT, ITimelockTarget {
    using SafeMath for uint256;

    string public constant name = "UMT";
    ...
    // by disabling the selling of UMT
    ...
    // - only allowing removal of UMT liquidity and no other actions
    // during the migration phase
    mapping (address => bool) public allowedMsgSenders;

    modifier onlyGov() {
        require(msg.sender == gov, "UMT: forbidden");
        _;
    }

    modifier onlyAdmin() {
        require(admins[msg.sender], "UMT: forbidden");
        _;
    }
    ...
    function setNextMigrationTime(uint256 _migrationTime) external onlyGov {
        require(_migrationTime > migrationTime, "UMT: invalid _migrationTime");
        migrationTime = _migrationTime;
    }

    function beginMigration() external override onlyAdmin {
        require(block.timestamp > migrationTime, "UMT: migrationTime not yet passed");
    ...
    function transferFrom(address _sender, address _recipient, uint256 _amount) external override returns (bool) {
        uint256 nextAllowance = allowances[_sender][msg.sender].sub(_amount, "UMT: transfer amount exceeds allowance");
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) private {
        require(_sender != address(0), "UMT: transfer from the zero address");
        require(_recipient != address(0), "UMT: transfer to the zero address");

        if (hasActiveMigration) {
            require(allowedMsgSenders[msg.sender], "UMT: forbidden msg.sender");
            require(!blockedRecipients[_recipient], "UMT: forbidden recipient");
        }

        balances[_sender] = balances[_sender].sub(_amount, "UMT: transfer amount exceeds balance");
    ...
    function _mint(address _account, uint256 _amount) private {
        require(_account != address(0), "UMT: mint to the zero address");
    ...
    function _approve(address _owner, address _spender, uint256 _amount) private {
        require(_owner != address(0), "UMT: approve from the zero address");
        require(_spender != address(0), "UMT: approve to the zero address");
