...
import "./interfaces/IUnityIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IUnityMigrator.sol";

contract UnityMigrator is ReentrancyGuard, IUnityMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "UnityMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "UnityMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "UnityMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "UnityMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "UnityMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "UnityMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "UnityMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "UnityMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "UnityMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "UnityMigrator: token not whitelisted");
        require(_tokenAmount > 0, "UnityMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "UnityMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(unityPrice);
        require(mintAmount > 0, "UnityMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "UnityMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "UnityMigrator: invalid tokenA");
            require(tokenB != address(0), "UnityMigrator: invalid tokenB");
        ...
        IUnityIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "UnityMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "UnityMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "UnityMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "UnityMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("UnityMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "UnityMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "UnityMigrator: invalid _action");
