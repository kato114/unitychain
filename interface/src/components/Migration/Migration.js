...
import UnityMigrator from "abis/UnityMigrator.json";
...
const decimals = 6;
const unityPrice = bigNumberify(2 * precision);
const tokens = [
  {
    name: "UMT",
    symbol: "UMT",
    address: getContract(CHAIN_ID, "UMT"),
    price: bigNumberify(10.97 * precision),
    iouToken: getContract(CHAIN_ID, "UMT_UNITY_IOU"),
    cap: MaxUint256,
    bonus: 0,
  },
  {
    name: "xUMT",
    symbol: "xUMT",
    address: getContract(CHAIN_ID, "XUMT"),
    price: bigNumberify(90.31 * precision),
    iouToken: getContract(CHAIN_ID, "XUMT_UNITY_IOU"),
    cap: MaxUint256,
    bonus: 0,
  },
  {
    name: "UMT-USDG",
    symbol: "LP",
    address: getContract(CHAIN_ID, "UMT_USDG_PAIR"),
    price: bigNumberify(parseInt(6.68 * precision)),
    iouToken: getContract(CHAIN_ID, "UMT_USDG_UNITY_IOU"),
  ...
  },
  {
    name: "xUMT-USDG",
    symbol: "LP",
    address: getContract(CHAIN_ID, "XUMT_USDG_PAIR"),
    price: bigNumberify(parseInt(19.27 * precision)),
    iouToken: getContract(CHAIN_ID, "XUMT_USDG_UNITY_IOU"),
...
const unityMigratorAddress = getContract(CHAIN_ID, "UnityMigrator");
...
function MigrationModal(props) {
  ...
  const { data: tokenAllowance, mutate: updateTokenAllowance } = useSWR(
    [active, CHAIN_ID, token.address, "allowance", account, unityMigratorAddress],
    {
  ...
  if (amount) {
    baseAmount = amount.mul(token.price).div(unityPrice);
    bonusAmount = baseAmount.mul(token.bonus).div(100);
    totalAmount = baseAmount.add(bonusAmount);

    baseAmountUsd = baseAmount.mul(unityPrice);
    bonusAmountUsd = bonusAmount.mul(unityPrice);
    totalAmountUsd = totalAmount.mul(unityPrice);
  }
  ...
  const onClickPrimary = () => {
    ...
    setIsMigrating(true);
    const contract = new ethers.Contract(unityMigratorAddress, UnityMigrator.abi, library.getSigner());
    contract
        ...
        <div className="MigrationModal-info-box">
          <div className="App-info-row">
            ...
            <div className="align-right">
              {baseAmount &&
                `${formatAmount(baseAmount, 18, 4, true)} $UNITY ($${formatAmount(
                  baseAmountUsd,
                  18 + decimals,
                  2,
                  true
          ...
          {token.bonus > 0 && (
            <div className="App-info-row">
              ...
              <div className="align-right">
                {bonusAmount &&
                  `${formatAmount(bonusAmount, 18, 4, true)} $UNITY ($${formatAmount(
                    bonusAmountUsd,
                    18 + decimals,
                    2,
                    true
          ...
          {token.bonus > 0 && (
            <div className="App-info-row">
              ...
              <div className="align-right">
                {totalAmount &&
                  `${formatAmount(totalAmount, 18, 4, true)} $UNITY ($${formatAmount(
                    totalAmountUsd,
                    18 + decimals,
                    2,
                    true
...
export default function Migration() {
  ...
  const { data: migratedAmounts, mutate: updateMigratedAmounts } = useSWR(
    ["Migration:migratedAmounts", CHAIN_ID, unityMigratorAddress, "getTokenAmounts"],
    {
      fetcher: contractFetcher(library, UnityMigrator, [tokenAddresses]),
    }
  );

  let unityBalance;
  let totalMigratedUnity;
  let totalMigratedUsd;

  if (iouBalances) {
    unityBalance = bigNumberify(0);
    totalMigratedUnity = bigNumberify(0);

    for (let i = 0; i < iouBalances.length / 2; i++) {
      unityBalance = unityBalance.add(iouBalances[i * 2]);
      totalMigratedUnity = totalMigratedUnity.add(iouBalances[i * 2 + 1]);
    }

    totalMigratedUsd = totalMigratedUnity.mul(unityPrice);
  }
  ...
  return (
    <div className="Migration Page">
      ...
      <div className="Migration-note">
        <Trans>Your wallet: {formatAmount(unityBalance, 18, 4, true)}</Trans> $UNITY
      </div>
      <div className="Migration-note">
        <Trans>
          Please read the{" "}
          <ExternalLink href="https://umtprotocol.medium.com/umt-unity-migration-now-live-2ba999d208dd">
            Medium post
          </ExternalLink>{" "}
          before migrating.
        </Trans>
