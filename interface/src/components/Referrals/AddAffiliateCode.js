...
function AddAffiliateCode({
  ...
  return (
    <div className="referral-card section-center mt-medium bg-red-bottom-left">
      <h2 className="title">
...
export function AffiliateCodeForm({
  ...
  async function handleSubmit(event) {
    ...
    if (takenStatus === "none" || takenStatus === "other") {
      const ownerOnOtherNetwork = takenInfo[chainId === ARBITRUM ? "ownerArbitrum" : "ownerArbitrum"];
      try {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button className="default-btn" type="submit" disabled={!isPrimaryEnabled()}>
        {getPrimaryText()}
      </button>
    </form>
  );
