...
export async function getReferralCodeTakenStatus(account, referralCode, chainId) {
  ...
  //ownerBsc, ownerMatic, ownerOptimism,
  const [ownerArbitrum] = await Promise.all([
    // getReferralCodeOwner(BSC, referralCodeBytes32),
    // getReferralCodeOwner(POLYGON, referralCodeBytes32),
    // getReferralCodeOwner(OPTIMISM, referralCodeBytes32),
    getReferralCodeOwner(ARBITRUM, referralCodeBytes32),
  ]);

  // const takenOnBsc = !isAddressZero(ownerBsc) && (ownerBsc !== account || (ownerBsc === account && chainId === BSC));
  // const takenOnMatic =
  //   !isAddressZero(ownerMatic) && (ownerMatic !== account || (ownerMatic === account && chainId === POLYGON));
  // const takenOnOptimism =
  //   !isAddressZero(ownerOptimism) && (ownerOptimism !== account || (ownerOptimism === account && chainId === OPTIMISM));
  const takenOnArbitrum =
  ...
  const referralCodeTakenInfo = {
    ...
    // [OPTIMISM]: takenOnOptimism,
    [ARBITRUM]: takenOnArbitrum,
    both: takenOnArbitrum, //takenOnBsc && takenOnMatic && takenOnOptimism &&
    // ownerBsc,
    // ownerMatic,
    // ownerOptimism,
    ownerArbitrum,
  ...
  if (chainId === ARBITRUM ? referralCodeTakenInfo[ARBITRUM] : referralCodeTakenInfo[ARBITRUM]) {
...
export function getTwitterShareUrl(referralCode) {
  const message = ["Trying out trading on @unitychain, up to 100x leverage on $BTC, $ETH 📈", "For fee discounts use:"];
