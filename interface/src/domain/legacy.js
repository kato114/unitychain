...
import { ARBITRUM, getConstant, getHighExecutionFee } from "config/chains"; //POLYGON, OPTIMISM, BSC,
...
import {
  ...
  optimismGraphClientForTrades,
  arbitrumGraphClientForTrades,
...
export function useAllOrdersStats(chainId) {
  ...
  useEffect(() => {
    const graphClient = getUnityGraphClient(chainId);
    if (graphClient) {
...
export function usePositionStates(chainId) {
  ...
  useEffect(() => {
    ...
    if (!arbitrumGraphClientForTrades) {
      return;
    }
...
export function useAllPositions(chainId, library) {
  ...
  useEffect(() => {
    arbitrumGraphClientForTrades.query({ query }).then(setRes).catch(console.warn);
  }, [setRes, query]);
...
export function useAllOrders(chainId, library) {
  ...
  useEffect(() => {
    getUnityGraphClient(chainId).query({ query }).then(setRes);
  }, [setRes, query, chainId]);
...
export function useMinExecutionFee(library, active, chainId, infoTokens) {
  ...
  // // multiplier for Optimism is just the average gas usage
  // if (chainId === OPTIMISM) {
  //   multiplier = 700000;
  // }

  // multiplier for Arbitrum is just the average gas usage
  if (chainId === ARBITRUM) {
    multiplier = 700000;
  }
...
export function useStakedUnitySupply(library, active) {
  // const unityAddressBsc = getContract(BSC, "UNITY");
  // const stakedUnityTrackerAddressBsc = getContract(BSC, "StakedUnityTracker");

  // const { data: bscData, mutate: bscMutate } = useSWR(
  //   [`StakeV2:stakedUnitySupply:${active}`, BSC, unityAddressBsc, "balanceOf", stakedUnityTrackerAddressBsc],
  //   {
  //     fetcher: contractFetcher(library, Token),
  //   }
  // );

  // const unityAddressMatic = getContract(POLYGON, "UNITY");
  // const stakedUnityTrackerAddressMatic = getContract(POLYGON, "StakedUnityTracker");

  // const { data: maticData, mutate: maticMutate } = useSWR(
  //   [`StakeV2:stakedUnitySupply:${active}`, POLYGON, unityAddressMatic, "balanceOf", stakedUnityTrackerAddressMatic],
  //   {
  //     fetcher: contractFetcher(undefined, Token),
  //   }
  // );

  // const unityAddressOptimism = getContract(OPTIMISM, "UNITY");
  // const stakedUnityTrackerAddressOptimism = getContract(OPTIMISM, "StakedUnityTracker");

  // const { data: optimismData, mutate: optimismMutate } = useSWR(
  //   [
  //     `StakeV2:stakedUnitySupply:${active}`,
  //     OPTIMISM,
  //     unityAddressOptimism,
  //     "balanceOf",
  //     stakedUnityTrackerAddressOptimism,
  //   ],
  //   {
  //     fetcher: contractFetcher(library, Token),
  //   }
  ...
  const unityAddressArbitrum = getContract(ARBITRUM, "UNITY");
  const stakedUnityTrackerAddressArbitrum = getContract(ARBITRUM, "StakedUnityTracker");

  const { data: arbitrumData, mutate: arbitrumMutate } = useSWR(
    [
      `StakeV2:stakedUnitySupply:${active}`,
      ARBITRUM,
      unityAddressArbitrum,
      "balanceOf",
      stakedUnityTrackerAddressArbitrum,
    ],
    {
      fetcher: contractFetcher(library, Token),
    }
  ...
  let data = arbitrumData;
  // if (bscData && maticData && optimismData && arbitrumData) {
  //   data = bscData.add(maticData);
  //   data = data.add(optimismData);
  //   data = data.add(arbitrumData);
  // }
...
export function useHasOutdatedUi() {
  ...
  return { data: false, mutate }; //hasOutdatedUi
}

export function useUnityPrice(chainId, libraries, active) {
  // const { data: unityPriceFromBsc, mutate: mutateFromBsc } = useUnityPriceFromBsc(
  //   libraries && libraries.bsc ? libraries.bsc : undefined,
  //   active
  // );
  // const { data: unityPriceFromPolygon, mutate: mutateFromPolygon } = useUnityPriceFromPolygon();
  // const { data: unityPriceFromOptimism, mutate: mutateFromOptimism } = useUnityPriceFromOptimism(
  //   libraries && libraries.optimism ? libraries.optimism : undefined,
  //   active
  // );
  const { data: unityPriceFromArbitrum, mutate: mutateFromArbitrum } = useUnityPriceFromArbitrum(
    libraries && libraries.arbitrum ? libraries.arbitrum : undefined,
    active
  );

  const unityPrice = unityPriceFromArbitrum;
  // chainId === BSC
  //   ? unityPriceFromBsc
  //   : chainId === POLYGON
  //   ? unityPriceFromPolygon
  //   : chainId === OPTIMISM
  //   ? unityPriceFromOptimism
  //   : unityPriceFromArbitrum;
  const mutate = useCallback(() => {
    // mutateFromPolygon();
    // mutateFromBsc();
    // mutateFromOptimism();
    mutateFromArbitrum();
  }, [mutateFromArbitrum]); //mutateFromPolygon, mutateFromBsc, mutateFromOptimism,
...
// use only the supply endpoint on bsc, it includes the supply on polygon
export function useTotalUnitySupply(CHAIN) {
  const { data: unitySupply } = useSWR(
    [`StakeV2:totalSupply:${CHAIN}`, CHAIN, getContract(CHAIN, "UNITY"), "totalSupply"],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
...
export function useTotalUnityStaked() {
  // const stakedUnityTrackerAddressBsc = getContract(BSC, "StakedUnityTracker");
  // const stakedUnityTrackerAddressMatic = getContract(POLYGON, "StakedUnityTracker");
  // const stakedUnityTrackerAddressOptimism = getContract(OPTIMISM, "StakedUnityTracker");
  const stakedUnityTrackerAddressArbitrum = getContract(ARBITRUM, "StakedUnityTracker");
  let totalStakedUnity = useRef(bigNumberify(0));
  // const { data: stakedUnitySupplyBsc, mutate: updateStakedUnitySupplyBsc } = useSWR(
  //   [`StakeV2:stakedUnitySupply:${BSC}`, BSC, getContract(BSC, "UNITY"), "balanceOf", stakedUnityTrackerAddressBsc],
  //   {
  //     fetcher: contractFetcher(undefined, Token),
  //   }
  // );
  // const { data: stakedUnitySupplyMatic, mutate: updateStakedUnitySupplyMatic } = useSWR(
  //   [
  //     `StakeV2:stakedUnitySupply:${POLYGON}`,
  //     POLYGON,
  //     getContract(POLYGON, "UNITY"),
  //     "balanceOf",
  //     stakedUnityTrackerAddressMatic,
  //   ],
  //   {
  //     fetcher: contractFetcher(undefined, Token),
  //   }
  // );
  // const { data: stakedUnitySupplyOptimism, mutate: updateStakedUnitySupplyOptimism } = useSWR(
  //   [
  //     `StakeV2:stakedUnitySupply:${OPTIMISM}`,
  //     OPTIMISM,
  //     getContract(OPTIMISM, "UNITY"),
  //     "balanceOf",
  //     stakedUnityTrackerAddressOptimism,
  //   ],
  //   {
  ...
  //   }
  // );
  const { data: stakedUnitySupplyArbitrum, mutate: updateStakedUnitySupplyArbitrum } = useSWR(
    [
      `StakeV2:stakedUnitySupply:${ARBITRUM}`,
      ARBITRUM,
      getContract(ARBITRUM, "UNITY"),
      "balanceOf",
      stakedUnityTrackerAddressArbitrum,
    ],
    {
  ...
  const mutate = useCallback(() => {
    // updateStakedUnitySupplyBsc();
    // updateStakedUnitySupplyMatic();
    // updateStakedUnitySupplyOptimism();
    updateStakedUnitySupplyArbitrum();
  }, [
    // updateStakedUnitySupplyBsc,
    // updateStakedUnitySupplyMatic,
    // updateStakedUnitySupplyOptimism,
  ...
  totalStakedUnity.current = stakedUnitySupplyArbitrum;

  // if (stakedUnitySupplyBsc && stakedUnitySupplyMatic && stakedUnitySupplyOptimism && stakedUnitySupplyArbitrum) {
  //   let total = bigNumberify(stakedUnitySupplyBsc)
  //     .add(bigNumberify(stakedUnitySupplyMatic))
  //     .add(bigNumberify(stakedUnitySupplyOptimism))
  //     .add(bigNumberify(stakedUnitySupplyArbitrum));
  //   totalStakedUnity.current = total;
  // }

  return {
    // matic: stakedUnitySupplyMatic,
    // bsc: stakedUnitySupplyBsc,
    // optimism: stakedUnitySupplyOptimism,
    arbitrum: stakedUnitySupplyArbitrum,
    total: stakedUnitySupplyArbitrum, //totalStakedUnity.current,
    mutate,
  };
}

export function useTotalUnityInLiquidity() {
  // let poolAddressBsc = getContract(BSC, "PancakeswapUnityBnbPool");
  // let poolAddressMatic = getContract(POLYGON, "TraderJoeUnityMaticPool");
  // let poolAddressOptimism = getContract(OPTIMISM, "UniswapUnityEthPool");
  let poolAddressArbitrum = getContract(ARBITRUM, "UniswapUnityEthPool");
  let totalUNITY = useRef(bigNumberify(0));

  // const { data: unityInLiquidityOnBsc, mutate: mutateUNITYInLiquidityOnBsc } = useSWR(
  //   [`StakeV2:unityInLiquidity:${BSC}`, BSC, getContract(BSC, "UNITY"), "balanceOf", poolAddressBsc],
  //   {
  //     fetcher: contractFetcher(undefined, Token),
  //   }
  // );
  // const { data: unityInLiquidityOnMatic, mutate: mutateUNITYInLiquidityOnMatic } = useSWR(
  //   [`StakeV2:unityInLiquidity:${POLYGON}`, POLYGON, getContract(POLYGON, "UNITY"), "balanceOf", poolAddressMatic],
  //   {
  //     fetcher: contractFetcher(undefined, Token),
  //   }
  // );
  // const { data: unityInLiquidityOnOptimism, mutate: mutateUNITYInLiquidityOnOptimism } = useSWR(
  //   [
  //     `StakeV2:unityInLiquidity:${OPTIMISM}`,
  //     OPTIMISM,
  //     getContract(OPTIMISM, "UNITY"),
  //     "balanceOf",
  //     poolAddressOptimism,
  ...
  //   }
  // );
  const { data: unityInLiquidityOnArbitrum, mutate: mutateUNITYInLiquidityOnArbitrum } = useSWR(
    [
      `StakeV2:unityInLiquidity:${ARBITRUM}`,
      ARBITRUM,
      getContract(ARBITRUM, "UNITY"),
      "balanceOf",
      poolAddressArbitrum,
  ...
  const mutate = useCallback(() => {
    // mutateUNITYInLiquidityOnBsc();
    // mutateUNITYInLiquidityOnMatic();
    // mutateUNITYInLiquidityOnOptimism();
    mutateUNITYInLiquidityOnArbitrum();
  }, [
    // mutateUNITYInLiquidityOnBsc,
    // mutateUNITYInLiquidityOnMatic,
    // mutateUNITYInLiquidityOnOptimism,
  ...
  totalUNITY.current = unityInLiquidityOnArbitrum;
  // if (unityInLiquidityOnMatic && unityInLiquidityOnBsc && unityInLiquidityOnOptimism && unityInLiquidityOnArbitrum) {
  //   let total = bigNumberify(unityInLiquidityOnBsc)
  //     .add(unityInLiquidityOnMatic)
  //     .add(unityInLiquidityOnOptimism)
  //     .add(unityInLiquidityOnArbitrum);
  //   totalUNITY.current = total;
  // }
  return {
    // matic: unityInLiquidityOnMatic,
    // bsc: unityInLiquidityOnBsc,
    // optimism: unityInLiquidityOnOptimism,
    arbitrum: unityInLiquidityOnArbitrum,
    total: totalUNITY.current,
    mutate,
  };
}

// function useUnityPriceFromPolygon() {
//   const poolAddress = getContract(POLYGON, "TraderJoeUnityMaticPool");

//   const { data, mutate: updateReserves } = useSWR(
//     ["TraderJoeUnityMaticReserves", POLYGON, poolAddress, "getReserves"],
//     {
//       fetcher: contractFetcher(undefined, UniswapV2),
//     }
//   );
//   const { _reserve1: unityReserve, _reserve0: maticReserve } = data || {};
...
//   let unityPrice;
//   if (maticReserve && unityReserve && maticPrice) {
//     unityPrice = maticReserve.mul(PRECISION).div(unityReserve).mul(maticPrice).div(PRECISION);
//   }
...
//   return { data: unityPrice, mutate };
// }

// function useUnityPriceFromOptimism(library, active) {
//   const poolAddress = getContract(OPTIMISM, "UniswapUnityEthPool");
//   const { data: uniPoolSlot0, mutate: updateUniPoolSlot0 } = useSWR(
//     [`StakeV2:uniPoolSlot0:${active}`, OPTIMISM, poolAddress, "slot0"],
//     {
//       fetcher: contractFetcher(library, UniPool),
//     }
//   );

//   const vaultAddress = getContract(OPTIMISM, "Vault");
//   const ethAddress = getTokenBySymbol(OPTIMISM, "WETH").address;
//   const { data: ethPrice, mutate: updateEthPrice } = useSWR(
//     [`StakeV2:ethPrice:${active}`, OPTIMISM, vaultAddress, "getMinPrice", ethAddress],
//     {
...
//       const tokenA = new UniToken(OPTIMISM, ethAddress, 18, "SYMBOL", "NAME");

//       const unityAddress = getContract(OPTIMISM, "UNITY");
//       const tokenB = new UniToken(OPTIMISM, unityAddress, 18, "SYMBOL", "NAME");
...
function useUnityPriceFromArbitrum(library, active) {
  const poolAddress = getContract(ARBITRUM, "UniswapUnityEthPool");
  const { data: uniPoolSlot0, mutate: updateUniPoolSlot0 } = useSWR(
  ...
  const unityPrice = useMemo(() => {
    if (uniPoolSlot0 && ethPrice) {
      ...
      const unityAddress = getContract(ARBITRUM, "UNITY");
      const tokenB = new UniToken(ARBITRUM, unityAddress, 18, "SYMBOL", "NAME");
...
// function useUnityPriceFromBsc(library, active) {
//   const poolAddress = getContract(BSC, "PancakeswapUnityBnbPool");

//   const { data, mutate: updateReserves } = useSWR(["PancakeswapUnityBnbReserves", BSC, poolAddress, "getReserves"], {
//     fetcher: contractFetcher(undefined, UniswapV2),
//   });
//   const { _reserve1: unityReserve, _reserve0: bnbReserve } = data || {};

//   const vaultAddress = getContract(BSC, "Vault");
//   const ethAddress = getTokenBySymbol(BSC, "WBNB").address;

//   const { data: bnbPrice, mutate: updateBnbPrice } = useSWR(
//     [`StakeV2:bnbPrice:${active}`, BSC, vaultAddress, "getMinPrice", ethAddress],
//     {
//       fetcher: contractFetcher(library, Vault),
//     }
//   );
...
//   const PRECISION = bigNumberify(10).pow(18);
//   let unityPrice;
//   if (bnbReserve && unityReserve && bnbPrice) {
//     unityPrice = bnbReserve.mul(PRECISION).div(unityReserve).mul(bnbPrice).div(PRECISION);
//   }

//   const mutate = useCallback(() => {
//     updateReserves(undefined, true);
//     updateBnbPrice(undefined, true);
//   }, [updateReserves, updateBnbPrice]);
...
export function useFeesData({ chainId = ARBITRUM, from = FIRST_DATE_TS, to = NOW_TS } = {}) {
...
export function useVolumeData({ chainId = ARBITRUM, from = FIRST_DATE_TS, to = NOW_TS } = {}) {
  const PROPS = "margin liquidation swap mint burn".split(" ");
  // const timestampProp = chainName === "bsc" ? "id" : "timestamp"
...
export function useTradersData({ chainId = ARBITRUM, from = FIRST_DATE_TS, to = NOW_TS } = {}) {
  const PROPS = "margin liquidation swap mint burn".split(" ");
  // const timestampProp = chainName === "bsc" ? "id" : "timestamp"
  const timestampProp = "timestamp";
  const [graphData, setRes] = useState();
  ...
  const data = useMemo(() => {
    ...
    ret = sortBy(graphData.data.tradingStats, timestampProp).map((dataItem) => {
