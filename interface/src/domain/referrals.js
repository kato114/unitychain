...
import {
  ...
  polygonReferralsGraphClient,
  optimismReferralsGraphClient,
  arbitrumReferralsGraphClient,
} from "lib/subgraph/clients"; // bscReferralsGraphClient,
...
const ACTIVE_CHAINS = [ARBITRUM]; //BSC, POLYGON, OPTIMISM,
...
function getGraphClient(chainId) {
  ...
  //   return optimismReferralsGraphClient;
  ...
  useEffect(() => {
    async function main() {
      const [arbitrumCodes] = await Promise.all(
        //bscCodes, polygonCodes, optimismCodes,
      ...
      const [codeOwnersOnArbitrum = []] = await Promise.all([
        //codeOwnersOnMatic = [], codeOwnersOnBsc = [], codeOwnersOnOptimism = [],
        // getCodeOwnersData(POLYGON, account, polygonCodes),
        // getCodeOwnersData(BSC, account, bscCodes),
        // getCodeOwnersData(OPTIMISM, account, optimismCodes),
        getCodeOwnersData(ARBITRUM, account, arbitrumCodes),
      ]);

      setData({
        // [POLYGON]: codeOwnersOnMatic.reduce((acc, cv) => {
        //   acc[cv.code] = cv;
        //   return acc;
        // }, {}),
        // [BSC]: codeOwnersOnBsc.reduce((acc, cv) => {
        //   acc[cv.code] = cv;
        //   return acc;
        // }, {}),
        // [OPTIMISM]: codeOwnersOnOptimism.reduce((acc, cv) => {
        //   acc[cv.code] = cv;
        //   return acc;
        // }, {}),
