...
export const ULP_COOLDOWN_DURATION = 15 * 60;
...
export function getBalanceAndSupplyData(balances) {
  ...
  const keys = ["unity", "esUnity", "ulp", "stakedUnityTracker"];
  const balanceData = {};
  const supplyData = {};
  const propsLength = 2;
...
export function getDepositBalanceData(depositBalances) {
  ...
  const keys = [
    "unityInStakedUnity",
    "esUnityInStakedUnity",
    "stakedUnityInBonusUnity",
...
export function getStakingData(stakingInfo) {
  ...
  const keys = ["stakedUnityTracker", "bonusUnityTracker", "feeUnityTracker", "stakedUlpTracker", "feeUlpTracker"];
  const data = {};
  const propsLength = 5;
...
export function getProcessedData(
  ...
  data.unityBalance = balanceData.unity;
  data.unityBalanceUsd = balanceData.unity.mul(unityPrice).div(expandDecimals(1, 18));

  data.unitySupply = bigNumberify(unitySupply);

  data.unitySupplyUsd = data.unitySupply.mul(unityPrice).div(expandDecimals(1, 18));
  data.stakedUnitySupply = stakedUnitySupply;
  data.stakedUnitySupplyUsd = stakedUnitySupply.mul(unityPrice).div(expandDecimals(1, 18));
  data.unityInStakedUnity = depositBalanceData.unityInStakedUnity;
  data.unityInStakedUnityUsd = depositBalanceData.unityInStakedUnity.mul(unityPrice).div(expandDecimals(1, 18));

  data.esUnityBalance = balanceData.esUnity;
  data.esUnityBalanceUsd = balanceData.esUnity.mul(unityPrice).div(expandDecimals(1, 18));

  data.stakedUnityTrackerSupply = supplyData.stakedUnityTracker;
  data.stakedUnityTrackerSupplyUsd = supplyData.stakedUnityTracker.mul(unityPrice).div(expandDecimals(1, 18));
  data.stakedEsUnitySupply = data.stakedUnityTrackerSupply.sub(data.stakedUnitySupply);
  data.stakedEsUnitySupplyUsd = data.stakedEsUnitySupply.mul(unityPrice).div(expandDecimals(1, 18));

  data.esUnityInStakedUnity = depositBalanceData.esUnityInStakedUnity;
  data.esUnityInStakedUnityUsd = depositBalanceData.esUnityInStakedUnity.mul(unityPrice).div(expandDecimals(1, 18));

  data.bnUnityInFeeUnity = depositBalanceData.bnUnityInFeeUnity;
  data.bonusUnityInFeeUnity = depositBalanceData.bonusUnityInFeeUnity;
  data.feeUnitySupply = stakingData.feeUnityTracker.totalSupply;
  data.feeUnitySupplyUsd = data.feeUnitySupply.mul(unityPrice).div(expandDecimals(1, 18));

  data.stakedUnityTrackerRewards = stakingData.stakedUnityTracker.claimable;
  data.stakedUnityTrackerRewardsUsd = stakingData.stakedUnityTracker.claimable
    .mul(unityPrice)
    .div(expandDecimals(1, 18));

  data.bonusUnityTrackerRewards = stakingData.bonusUnityTracker.claimable;

  data.feeUnityTrackerRewards = stakingData.feeUnityTracker.claimable;
  data.feeUnityTrackerRewardsUsd = stakingData.feeUnityTracker.claimable
    .mul(nativeTokenPrice)
    .div(expandDecimals(1, 18));

  data.boostBasisPoints = bigNumberify(0);
  if (data && data.bnUnityInFeeUnity && data.bonusUnityInFeeUnity && data.bonusUnityInFeeUnity.gt(0)) {
    data.boostBasisPoints = data.bnUnityInFeeUnity.mul(BASIS_POINTS_DIVISOR).div(data.bonusUnityInFeeUnity);
  }

  data.stakedUnityTrackerAnnualRewardsUsd = stakingData.stakedUnityTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(unityPrice)
    .div(expandDecimals(1, 18));
  data.unityAprForEsUnity =
    data.stakedUnityTrackerSupplyUsd && data.stakedUnityTrackerSupplyUsd.gt(0)
      ? data.stakedUnityTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.stakedUnityTrackerSupplyUsd)
      : bigNumberify(0);
  data.feeUnityTrackerAnnualRewardsUsd = stakingData.feeUnityTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(nativeTokenPrice)
    .div(expandDecimals(1, 18));
  data.unityAprForNativeToken =
    data.feeUnitySupplyUsd && data.feeUnitySupplyUsd.gt(0)
      ? data.feeUnityTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.feeUnitySupplyUsd)
      : bigNumberify(0);
  data.unityBoostAprForNativeToken = data.unityAprForNativeToken.mul(data.boostBasisPoints).div(BASIS_POINTS_DIVISOR);
  data.unityAprTotal = data.unityAprForNativeToken.add(data.unityAprForEsUnity);
  data.unityAprTotalWithBoost = data.unityAprForNativeToken
    .add(data.unityBoostAprForNativeToken)
    .add(data.unityAprForEsUnity);
  data.unityAprForNativeTokenWithBoost = data.unityAprForNativeToken.add(data.unityBoostAprForNativeToken);

  data.totalUnityRewardsUsd = data.stakedUnityTrackerRewardsUsd.add(data.feeUnityTrackerRewardsUsd);
  ...
  data.stakedUlpTrackerRewardsUsd = stakingData.stakedUlpTracker.claimable.mul(unityPrice).div(expandDecimals(1, 18));
  ...
  data.ulpAprForNativeToken =
    data.ulpSupplyUsd && data.ulpSupplyUsd.gt(0)
      ...
      : bigNumberify(0);
  data.ulpAprTotal = data.ulpAprForNativeToken.add(data.ulpAprForEsUnity);

  data.totalUlpRewardsUsd = data.stakedUlpTrackerRewardsUsd.add(data.feeUlpTrackerRewardsUsd);

  data.totalEsUnityRewards = data.stakedUnityTrackerRewards.add(data.stakedUlpTrackerRewards);
  data.totalEsUnityRewardsUsd = data.stakedUnityTrackerRewardsUsd.add(data.stakedUlpTrackerRewardsUsd);

  data.unityVesterRewards = vestingData.unityVester.claimable;
  data.ulpVesterRewards = vestingData.ulpVester.claimable;
  data.totalVesterRewards = data.unityVesterRewards.add(data.ulpVesterRewards);
  data.totalVesterRewardsUsd = data.totalVesterRewards.mul(unityPrice).div(expandDecimals(1, 18));

  data.totalNativeTokenRewards = data.feeUnityTrackerRewards.add(data.feeUlpTrackerRewards);
  data.totalNativeTokenRewardsUsd = data.feeUnityTrackerRewardsUsd.add(data.feeUlpTrackerRewardsUsd);

  data.totalRewardsUsd = data.totalEsUnityRewardsUsd
...
export function isDevelopment() {
  return !window.location.host?.includes("unity.io") && !window.location.host?.includes("ipfs.io");
}
...
export function getHomeUrl() {
  ...
  return "https://app.utrade.exchange/";
}
...
export function getRootShareApiUrl() {
  if (isLocal()) {
    return "https://unitys.vercel.app";
  }

  return "https://share.unity.io";
}
...
export function getTradePageUrl() {
  ...
  return "https://app.utrade.exchange/#/trade";
}
