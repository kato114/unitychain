...
export default function DashboardV1() {
  ...
  const umtSupply = bigNumberify("1000000000000000000000000").sub(bigNumberify("598530625016359222222472"));

  const readerAddress = getContract(chainId, "Reader");
  const ammFactoryAddressV2 = getContract(chainId, "AmmFactoryV2");
  const umtAddress = getContract(chainId, "UMT");
  const xumtAddress = getContract(chainId, "XUMT");
  ...
  const { data: pairInfo, mutate: updatePairInfo } = useSWR(
    ...
    {
      fetcher: contractFetcher(library, Reader, [[umtAddress, usdgAddress, xumtAddress, usdgAddress]]),
    }
  );
  ...
  const { data: xumtSupply, mutate: updateXumtSupply } = useSWR(
    ["Dashboard:xumtSupply", chainId, readerAddress, "getTokenSupply", xumtAddress],
    {
  ...
  if (pairInfo && pairInfo.length === 4) {
    umtPairInfo = [pairInfo[0], pairInfo[1]];
    xumtPairInfo = [pairInfo[2], pairInfo[3]];
  }

  let umtPrice;
  let umtMarketCap;
  if (umtPairInfo && umtSupply && umtPairInfo[1] && umtPairInfo[0] && umtPairInfo[0].gt(0)) {
    umtPrice = umtPairInfo[1].mul(PRECISION).div(umtPairInfo[0]);
    umtMarketCap = umtSupply.mul(umtPrice).div(expandDecimals(1, 18));
  }

  let xumtPrice;
  let xumtMarketCap;
  if (xumtPairInfo && xumtSupply && xumtPairInfo[1] && xumtPairInfo[0] && xumtPairInfo[0].gt(0)) {
    xumtPrice = xumtPairInfo[1].mul(PRECISION).div(xumtPairInfo[0]);
    xumtMarketCap = xumtSupply.mul(xumtPrice).div(expandDecimals(1, 18));
  }
      ...
      <div className="Dashboard-token-list-container">
        <div className="Dashboard-token-list Dashboard-list">
          <div className="Dashboard-token-card usdg App-card primary">
            <div className="Dashboard-token-title App-card-title">
              ...
              <div className="Dashboard-token-title-options">
                ...
                />
                <ExternalLink href="https://www.coingecko.com/en/coins/usd-umt">
          ...
          <div className="Dashboard-token-card App-card">
            <div className="Dashboard-token-title App-card-title">
              <div className="Dashboard-token-title-text">UMT</div>
              <div className="Dashboard-token-title-options">
                <img
                  src={metamaskImg}
                  alt="MetaMask"
                  onClick={() =>
                    addToken({
                      ...
                      info: {
                        decimals: 18,
                        imageUrl: "https://assets.coingecko.com/coins/images/14270/small/umt_logo.png",
                      },
                    })
                  }
                />
                <ExternalLink href="https://www.coingecko.com/en/coins/umt">
                  <img src={coingeckoImg} alt="CoinGecko" />
                </ExternalLink>
            ...
            <div className="Dashboard-token-card-bottom App-card-content">
              <div className="Dashboard-token-info App-card-row">
                ...
                <div>{formatAmount(umtSupply, 18, 0, true)} UMT</div>
              </div>
              <div className="App-card-row">
                <div className="label">
                  <Trans>Price</Trans>
                </div>
                <div>{formatAmount(umtPrice, USD_DECIMALS, 2, true)} USD</div>
              </div>
              <div className="App-card-row">
                <div className="label">
                  <Trans>Market Cap</Trans>
                </div>
                <div>{formatAmount(umtMarketCap, USD_DECIMALS, 0, true)} USD</div>
              </div>
            </div>
          </div>
          <div className="Dashboard-token-card App-card">
            <div className="Dashboard-token-title App-card-title">
              <div className="Dashboard-token-title-text">xUMT</div>
              <div className="Dashboard-token-title-options">
                <img
                  src={metamaskImg}
                  alt="MetaMask"
                  onClick={() =>
                    addToken({
                      ...
                      info: {
                        decimals: 18,
                        imageUrl: "https://assets.coingecko.com/coins/images/15888/small/xumt-transparent-yellow.png",
                      },
                    })
                  }
                />
                <ExternalLink href="https://www.coingecko.com/en/coins/xumt">
                  <img src={coingeckoImg} alt="CoinGecko" />
                </ExternalLink>
            ...
            <div className="Dashboard-token-card-bottom App-card-content">
              <div className="Dashboard-token-info App-card-row">
                ...
                <div>{formatAmount(xumtSupply, 18, 0, true)} xUMT</div>
              </div>
              <div className="App-card-row">
                <div className="label">
                  <Trans>Price</Trans>
                </div>
                <div>{formatAmount(xumtPrice, USD_DECIMALS, 2, true)} USD</div>
              </div>
              <div className="App-card-row">
                <div className="label">
                  <Trans>Market Cap</Trans>
                </div>
                <div>{formatAmount(xumtMarketCap, USD_DECIMALS, 0, true)} USD</div>
              </div>
            </div>
          </div>
          {infoTokens &&
